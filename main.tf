locals {
  ...
  span_destination_groups = [for group in try(local.access_policies.span.destination_groups, []) : {
  ...
  span_source_groups = [for group in try(local.access_policies.span.source_groups, []) : {
...
module "aci_vlan_pool" {
  ...
  for_each    = { for vp in try(local.access_policies.vlan_pools, []) : vp.name => vp if try(local.modules.aci_vlan_pool, true) }
...
module "aci_physical_domain" {
  ...
  for_each             = { for pd in try(local.access_policies.physical_domains, []) : pd.name => pd if try(local.modules.aci_physical_domain, true) }
...
module "aci_routed_domain" {
  ...
  for_each             = { for rd in try(local.access_policies.routed_domains, []) : rd.name => rd if try(local.modules.aci_routed_domain, true) }
...
module "aci_aaep" {
  ...
  for_each           = { for aaep in try(local.access_policies.aaeps, []) : aaep.name => aaep if try(local.modules.aci_aaep, true) }
...
module "aci_mst_policy" {
  ...
  for_each = { for mst in try(local.access_policies.switch_policies.mst_policies, []) : mst.name => mst if try(local.modules.aci_mst_policy, true) }
...
module "aci_access_leaf_switch_policy_group" {
  ...
  for_each                = { for pg in try(local.access_policies.leaf_switch_policy_groups, []) : pg.name => pg if try(local.modules.aci_access_leaf_switch_policy_group, true) }
...
module "aci_access_spine_switch_policy_group" {
  ...
  for_each    = { for pg in try(local.access_policies.spine_switch_policy_groups, []) : pg.name => pg if try(local.modules.aci_access_spine_switch_policy_group, true) }
...
module "aci_access_leaf_switch_profile_manual" {
  ...
  for_each = { for prof in try(local.access_policies.leaf_switch_profiles, []) : prof.name => prof if try(local.modules.aci_access_leaf_switch_profile, true) }
...
module "aci_access_spine_switch_profile_manual" {
  ...
  for_each = { for prof in try(local.access_policies.spine_switch_profiles, []) : prof.name => prof if try(local.modules.aci_access_spine_switch_profile, true) }
...
module "aci_lldp_policy" {
  ...
  for_each       = { for lldp in try(local.access_policies.interface_policies.lldp_policies, []) : lldp.name => lldp if try(local.modules.aci_lldp_policy, true) }
...
module "aci_link_level_policy" {
  ...
  for_each = { for llp in try(local.access_policies.interface_policies.link_level_policies, []) : llp.name => llp if try(local.modules.aci_link_level_policy, true) }
...
module "aci_port_channel_policy" {
  ...
  for_each             = { for pc in try(local.access_policies.interface_policies.port_channel_policies, []) : pc.name => pc if try(local.modules.aci_port_channel_policy, true) }
...
module "aci_port_channel_member_policy" {
  ...
  for_each = { for pcm in try(local.access_policies.interface_policies.port_channel_member_policies, []) : pcm.name => pcm if try(local.modules.aci_port_channel_member_policy, true) }
...
module "aci_l2_policy" {
  ...
  for_each         = { for l2 in try(local.access_policies.interface_policies.l2_policies, []) : l2.name => l2 if try(local.modules.aci_l2_policy, true) }
...
module "aci_storm_control_policy" {
  ...
  for_each                   = { for sc in try(local.access_policies.interface_policies.storm_control_policies, []) : sc.name => sc if try(local.modules.aci_storm_control_policy, true) }
...
module "aci_access_leaf_interface_policy_group" {
  ...
  for_each                   = { for pg in try(local.access_policies.leaf_interface_policy_groups, []) : pg.name => pg if try(local.modules.aci_access_leaf_interface_policy_group, true) }
...
module "aci_access_spine_interface_policy_group" {
  ...
  for_each          = { for pg in try(local.access_policies.spine_interface_policy_groups, []) : pg.name => pg if try(local.modules.aci_access_spine_interface_policy_group, true) }
...
module "aci_access_leaf_interface_selector_manual" {
  ...
  for_each              = { for selector in local.leaf_interface_selectors : selector.key => selector.value if try(local.modules.aci_access_leaf_interface_selector, true) }
...
module "aci_access_fex_interface_selector_manual" {
  ...
  for_each          = { for selector in local.fex_interface_selectors : selector.key => selector.value if try(local.modules.aci_access_fex_interface_selector, true) }
  interface_profile = each.value.profile_name
  name              = each.value.name
  policy_group      = each.value.policy_group
  policy_group_type = each.value.policy_group_type
...
module "aci_access_spine_interface_selector_manual" {
  ...
  for_each          = { for selector in local.spine_interface_selectors : selector.key => selector.value if try(local.modules.aci_access_spine_interface_selector, true) }
  interface_profile = each.value.profile_name
  name              = each.value.name
  policy_group      = each.value.policy_group
  port_blocks       = each.value.port_blocks
...
module "aci_qos" {
  ...
  count        = try(local.modules.aci_qos, true) == false ? 0 : 1
...
module "aci_access_span_filter_group" {
  ...
  for_each    = { for group in try(local.access_policies.span.filter_groups, []) : group.name => group if try(local.modules.aci_access_span_filter_group, true) }
...
module "aci_access_span_source_group" {
  ...
  for_each                = { for group in local.span_source_groups : group.name => group if try(local.modules.aci_access_span_source_group, true) }
  name                    = each.value.name
  description             = each.value.description
  admin_state             = each.value.admin_state
...
module "aci_vspan_destination_group" {
  ...
  for_each    = { for group in try(local.access_policies.vspan.destination_groups, []) : group.name => group if try(local.modules.aci_vspan_destination_group, true) }
...
module "aci_vspan_session" {
  ...
  for_each                = { for session in local.vspan_sessions : session.name => session if try(local.modules.aci_vspan_session, true) }
  name                    = each.value.name
  description             = each.value.description
  admin_state             = each.value.admin_state
  destination_name        = each.value.destination_name
